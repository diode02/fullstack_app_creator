...
class modelsCodeService {
  ...
  GetScheemaModelCode(currentTable, schemaTables, schemaRelations) {
    ...
    modelFileCode += this.getMongoDBScheema(
      currentTable,
      schemaTables,
      schemaRelations
    );
      ...
      for (var index in schemaRelations) {
        ...
        if (tblName == firstTable) {
          ...
          if (secondTable.toLowerCase() == "user") {
            //if second table is User
            secondTableColumnType = this.getMongoDBType("text"); //as all the fields are string type so direct text passed
          } else {
            //find the table in the schemaTables that user gave
            for (var tableId in schemaTables) {
              //first find secondTable from all the tables
              innerTableName = schemaTables[tableId].name;
              if (innerTableName == secondTable) {
                //if found
                innerColumns = schemaTables[tableId].columns;
                for (var colIndex in innerColumns) {
                  //traverse through its columns
                  if (innerColumns[colIndex].name == secondTableColumn) {
                    //if column name is equal get respective type
                    secondTableColumnType = this.getMongoDBType(
                      innerColumns[colIndex].type
                    );
                    break;
                  }
                }
