...
class routesCodeService {
  ...
  GetRouteModelCode(schemaTable, schemaRelations) {
    ...
    var routesFilesCode = this.getImports(tblName, schemaRelations); //imports
    routesFilesCode += this.getIndexRoute(tblName);
    routesFilesCode += this.getSpecificRecordRoute(
      schemaTable,
      schemaRelations
    );
    routesFilesCode += this.getPostRoute(schemaTable, schemaRelations);
    routesFilesCode += this.getDeleteRecordRoute(tblName);
    routesFilesCode += this.getPutRoute(schemaTable, schemaRelations);
    routesFilesCode += this.getExpots();
    return routesFilesCode;
  }
  ...
  getImports(tableName, schemaRelations) {
    ...
    imprtStatmnts += 'const auth = require("../src/middlewares/auth");\n';
    imprtStatmnts = imprtStatmnts.concat(
      "const " +
        tableName +
        ' = require("../src/models/' +
        tableName +
    ...
    if (schemaRelations != null) {
      for (var index in schemaRelations) {
        ...
        if (tableName == schemaRelations[index].firstTable) {
          imprtStatmnts +=
            "const " +
            schemaRelations[index].secondTable +
            '= require("../src/models/' +
            schemaRelations[index].secondTable +
            '.model");\n';
        }
  ...
  getIndexRoute(tblName) {
    let route = "";
    route += 'router.get("/", auth, async (req, res) => {\n';
    route = route.concat(
      "  try {\n" +
        "    const " +
        tblName +
  ...
  getSpecificRecordRoute(schemaTable, schemaRelations) {
    ...
    route += 'router.get("/:id", auth, async (req, res) => {\n';
    route = route.concat(
      "  try {\n" +
        "    const " +
        tblName.toLowerCase() +
    ...
    if (schemaRelations != null) {
      for (var index in schemaRelations) {
        ...
        if (tblName == firstTable) {
          // if its same as the currentTable i.e tblName then this relation belongs to current table
          secondTable = schemaRelations[index].secondTable;
          relationType = schemaRelations[index].relationType;
          if (
            relationType.toLowerCase() == "checkbox" ||
            relationType.toLowerCase() == "multiselect" ||
            relationType.toLowerCase() == "manytomany"
          ) {
            route = route.concat(
              "    let " +
                secondTable.toLowerCase() +
                "Ids = new Array();\n" +
                "    for(var i=0; i<" +
                firstTable.toLowerCase() +
                "." +
                secondTable +
                "s.length; i++){\n" +
                "      " +
    ...
    route = route.concat("        _id: " + tblName.toLowerCase() + "._id,\n");
    ...
    if (schemaRelations != null) {
      for (var index in schemaRelations) {
        ...
        if (tblName == firstTable) {
          ...
          relationType = schemaRelations[index].relationType;
          if (
            relationType.toLowerCase() == "select" ||
            relationType.toLowerCase() == "radio" ||
            relationType.toLowerCase() == "onetomany"
          ) {
            route = route.concat(
              "        " +
                secondTable +
  ...
  getPostRoute(schemaTable, schemaRelations) {
    ...
    route += 'router.post("/", auth, async (req, res) => {\n';
    route += "  try {\n";

    if (schemaRelations != null) {
      //if not passed by user
      for (var index in schemaRelations) {
        //so we have all the relations
        firstTable = schemaRelations[index].firstTable; // get each realtions firstTable
        if (tblName == firstTable) {
          // if its same as the currentTable i.e tblName then this relation belongs to current table
          secondTable = schemaRelations[index].secondTable;
    ...
    route +=
      "    const " + tblName.toLowerCase() + " = new " + tblName + " ({\n";
    for (var column in tblColumns) {
      route = route.concat(
        "        " +
          tblColumns[column].name +
  ...
  getDeleteRecordRoute(tblName) {
    let route = "";
    route += 'router.delete("/:id", auth, async (req, res) => {\n';
    route = route.concat(
      "  try {\n" +
        "    const removed" +
        tblName +
  ...
  getPutRoute(schemaTable, schemaRelations) {
    ...
    route += 'router.put("/:id", auth, async (req, res) => {\n';
    route += "  try {\n";
    ...
    route +=
      "    const updated" + tblName + " = await " + tblName + ".updateOne(\n";
