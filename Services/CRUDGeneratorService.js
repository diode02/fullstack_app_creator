...
const nativeGenerateService = require("./Native/NativeService");

let instance = null;
let frontEndGenerateServiceObj = null;
let backEndGenerateServiceObj = null;
let nativeGenerateServiceObj = null;
...
class CRUDGeneratorService {
  constructor() {
    ...
    nativeGenerateServiceObj = nativeGenerateService.getInstance();
  }
  ...
  generateCRUD(scheema, outputFolderLoc, onlyNodeApp) {
    //C:\Work\UNIRelated\FYP\pack\CRUD_Generator_React
    ...
    let appName,
      projectFolderName,
      ReactProjectFolderName,
      NodeProjectFolderName,
      NativeProjectFolderName,
      projectDatabaseName;

    if (applicationName) {
      //if user passed name for its application
      ...
      NativeProjectFolderName = applicationName + "NativeApp";
      ...
      CRUDConfigurations.NativeProjectFolderName = NativeProjectFolderName;
      CRUDConfigurations.NodeProjectFolderName = NodeProjectFolderName;
    } else {
      //use default or preset names
      ...
      NativeProjectFolderName =
        CRUDConfigurations.DefaultNativeProjectFolderName;
      ...
      CRUDConfigurations.ProjectFolderName =
        CRUDConfigurations.DefaultProjectFolderName;
      CRUDConfigurations.ProjectAppName =
        CRUDConfigurations.DefaultProjectAppName;
      CRUDConfigurations.ReactProjectFolderName =
        CRUDConfigurations.DefaultReactProjectFolderName;
      CRUDConfigurations.NativeProjectFolderName =
        CRUDConfigurations.DefaultNativeProjectFolderName;
      CRUDConfigurations.NodeProjectFolderName =
        CRUDConfigurations.DefaultNodeProjectFolderName;
    }

    if (databaseName) {
      //if user passed name for its project database
      projectDatabaseName = databaseName;
      //also setting values to crudConfig so it will be accessible accross the application
      CRUDConfigurations.ProjectDatabaseName = projectDatabaseName;
    } else if (applicationName) {
      //if user passed name for its project only so use that as DBName
      projectDatabaseName = applicationName;
      //also setting values to crudConfig so it will be accessible accross the application
      CRUDConfigurations.ProjectDatabaseName = projectDatabaseName;
    } else {
      //not passed any so use default or preset names
      projectDatabaseName = CRUDConfigurations.DefaultProjectDatabaseName;
      CRUDConfigurations.ProjectDatabaseName =
        CRUDConfigurations.DefaultProjectDatabaseName;
    }

    if (applicationTheme) {
      //if user passed name for its project theme
      //also setting values to crudConfig so it will be accessible accross the application
      CRUDConfigurations.ProjectThemeName = applicationTheme;
    } else {
      //use default or preset theme
      CRUDConfigurations.ProjectThemeName =
        CRUDConfigurations.DefaultProjectThemeName;
    }
    ...
    const ReactProjectFolderCompName =
      projectFolderPath + "/" + ReactProjectFolderName;
    const NativeProjectFolderCompName =
      projectFolderPath + "/" + NativeProjectFolderName;
    const NodeProjectFolderCompName =
      projectFolderPath + "/" + NodeProjectFolderName;
  ...
  validateScheema(scheema) {
    ...
    var crudGenSchema = {
      ...
      properties: {
        ...
        appTheme: {
          enum: ["dark", "defaultLight", "electricBlue"],
        },
        appDbName: {
          type: "string",
        },
    ...
    if (errors != "") {
      ...
      for (var tableId in schemaTables) {
        //makesure user cannot create a table with name "user".
        var tableName = schemaTables[tableId].name;
        if (tableName.toLowerCase() == "user") {
          errors +=
            '0: "' +
            tableName +
            '" is not a valid tableName as this will be generated automatically, please use something else.';
          return errors;
        }
      }
      ...
      for (var relationId in schemaRelations) {
        // in case of relation there are two tables first table makes relation with second table column
        ...
        var firstTableFound = false,
          secondTableFound = false,
          secondTableColumnFound = false;

        if (firstTable == secondTable) {
          //if user try to make a table relation with itself
          errors +=
            "0: A table cannot have relation with it self please use different table names on instance.appSchema.relations[" +
            relationId +
            "].";
          return errors;
        }

        if (firstTable.toLowerCase() == "user") {
          //user table cannot make relation with anyother table
          errors +=
            '0: "' +
            firstTable +
            "table cannot be used as firstTable or cannot make relation with other tables  on instance.appSchema.relations[" +
            relationId +
            "], please use any other table.";
          return errors;
        }
        ...
        for (var tableId in schemaTables) {
          ...
          if (tableName == secondTable) {
            ...
            for (var columnId in tableColumns) {
              //then iterate through its columns
              var columnName = tableColumns[columnId].name; // and get the name of each column
              if (columnName == secondTableColumn) {
                //check if it matches with any second table columnName
                secondTableColumnFound = true;
              }
            }

            if (!secondTableColumnFound) {
              //so column name is invalid
              errors +=
                "0: invalid column name used in secondTableColumn on instance.appSchema.relations[" +
                relationId +
                "], please specify name of column that exist in " +
                secondTable +
                " scheema.";
              return errors;
            }
          ...
          if (secondTable.toLowerCase() == "user") {
            //if other table try making realtion with user table
            secondTableFound = true;
            if (
              !(
                secondTableColumn == "name" ||
              ...
              errors +=
                ...
                "], " +
                secondTableColumn +
                " table only has name,email,password fields so use any of these.";
              return errors;
            }
            secondTableColumnFound = true;
          }
        ...
        if (!firstTableFound) {
          // firstTable not found
          errors +=
            "0: invalid table name used on firstTable of instance.appSchema.relations[" +
            relationId +
            "], please specify name of table that you provided in scheema.";
          return errors;
        }
        if (!secondTableFound) {
          // secondTable not found
          errors +=
            "0: invalid table name used on secondTable of instance.appSchema.relations[" +
            relationId +
            "], please specify name of table that you provided in scheema.";
          return errors;
        }
        if (!secondTableColumnFound) {
          // secondTable column not found
          errors +=
            "0: invalid column name used on secondTableColumn of instance.appSchema.relations[" +
            relationId +
            "], please specify name of column that exist in " +
            secondTable +
            ".";
        ...
        //and relation is not redefine from other side i.e for first->second we are getting second -> first
        for (var inneerRelationId in schemaRelations) {
          // in case of relation there are two tables first table makes relation with second table column
          ...
          if (relationId == inneerRelationId) {
            //if comparing with itself
            continue; //skip iteration
          } else if (
            (firstTable == innerFirstTable &&
              secondTable == innerSecondTable) ||
            (firstTable == innerSecondTable && secondTable == innerFirstTable)
          ) {
            errors +=
              "0: There can only be one relation between two tables, relation between " +
              firstTable +
              " and " +
              secondTable +
              " are defined twice on instance.appSchema.relations[" +
              relationId +
              "] and instance.appSchema.relations[" +
              inneerRelationId +
              "]";
            return errors;
          }
