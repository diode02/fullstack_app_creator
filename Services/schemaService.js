...
class schemaService {
  ...
  generateCRUDApp(scheemaJson, outputFolderpath, onlyNodeApp) {
    try {
      var schemaValidationErrors = CRUDGeneratorServiceObj.validateScheema(
        scheemaJson
      );
      if (schemaValidationErrors == "") {
        //if the scheema is in valid format so there will be no errors
        ...
        exec(
          "npx prettier --write ./AppGenerated/Ecomerace/EcomeraceReactApp/src/",
          (error, stdout, stderr) => {
            if (error) {
        ...
        this.createErrorsFile(
          outputFolderpath,
          "Schema Validation Failed because of following reasons :\n" +
